...
本轮以仓库正文（`讲义/专题/`、`讲义/历年卷/`、`讲义/其它/`）和最终 PDF 为兼容边界。`讲义/LALUbook.cls`、主文件 `讲义/LALU.tex`、答案模板 `讲义/LALU-answers.tex` 中仅用于支撑实现的 counter、helper、marker、生成文件协议和 href anchor **不是兼容接口**；只要正文无需改写、既有标签/引用语义和 PDF 可见行为不变，它们可以直接删除、改名或换成新 schema，不设过渡包装器。
...
## 七、`patterns.re` 历史正则审计

`patterns.re` 是历次人工校对留下的搜索/替换便笺，不是可以按文件顺序无审查执行的 formatter。本节只记录各组规则的适用场景和后续处置；本轮不执行这些替换，不修改 `patterns.re`，也不据此批量修改正文。

这些规则有两项共同限制。第一，反复使用的 `[\u10ff-\ufeff]` 并不是中文字符类，而是覆盖 U+10FF--U+FEFF 间多种文字、数学符号、标点和私用区的宽范围。第二，正则不能解析 TeX 的嵌套花括号、注释、数学模式或 verbatim 类环境，而其中多处 `.+` 又是贪婪匹配。新增或删除空行还可能产生或移除 `\par`，从而影响列表间距、首段行为和答案正文。因此，后续即使采用某条规则，也应把它改成只报告候选位置的 lint，逐类 review、提交和编译验证。

### 7.1 `line wrap`

- `([…])\n *([…]) -> $1$2` 用于删除中文文字、标点、`$`、`=` 等字符之间的硬换行；其范围也会命中对齐公式和 TikZ 坐标，甚至可能把相邻两行的 `$` 拼成 `$$`，不能全仓替换。
- 引用命令后换行、句点后换行、数字后换行，以及正文换行后接非 `begin`/`end`/`item` 命令的几条无 replacement 规则，意在定位被折行的连续语句。这些查询的命令名和参数匹配都很宽，且会命中多行公式，应只作为人工搜索提示。
- `\.\n *([…]) -> . $1` 只是把单换行规范成一个源码空格；TeX 原来的单换行通常已经等价于空格，而后继为标点或数学符号时，这一替换反而可能制造错误间距。

归类：源码 lint/人工排版检查，不进入模板层，也不作为自动修复。

### 7.2 `item wrap`

- `\item` 单独占行后接正文的规则把首段移到同一源码行，通常只是风格调整，但不覆盖以命令、花括号或可选标签开始的 item。
- 连续 `\item`、环境结束后接 `\item`、显示公式后接 `\item` 等规则会插入空行；`enumerate` 后首项以及空 `\item` 后接环境的规则又会删除换行或把两行合并。它们互相重叠并具有顺序依赖。
- 在 list 中插入空行会产生段落边界，可能改变 `\parsep`；把空项与下一 `\item` 合并还会生成难以辨认的 `\item \item`，掩盖源文件错误。

归类：如需统一源码风格，优先配置 `latexindent`；涉及段落边界的候选仍须人工 review，不批量修改正文。

### 7.3 `dot spacing`

- `\.([\u10ff-\ufeff]) -> . $1` 主要针对 `.故`、`.因此` 一类西文句点后紧接中文正文的情况，仓库中确有此类实例；但宽字符范围也包含中文标点和数学符号，可能在标点前或数学式内错误插空格。
- `\.\n *(\d) -> . $1` 将句点、换行、数字整理为同一行；原换行通常已经提供空格，且该规则无法判断是否遇到被错误拆行的小数。

归类：只报告“句点后直接接 Han 字符”的窄 lint，并排除数学、注释、verbatim 和标点；不使用现有宽范围直接替换。

### 7.4 `ref spacing`

搜索式试图覆盖 `\ref`、`\autoref`、`\eqref`、`\pageref`、`\nameref` 等 `*ref` 命令，并用 `[^h]` 避开 `\href`/`\hyperref`。其记录的 replacement 按文件内容实际只有 `$1`，没有尾随空格，因此当前是 no-op。即使补上空格也不正确：引用后是 `，`、`）` 等标点时不应插入；`\autoref` 常以数字结束而需要中西文间距，`\nameref` 却可能展开为中文标题并直接连接“一章”等中文文字。

归类：保留现有模板中针对 `\autoref` 的后继 token 分类。若以后扩展到 `\eqref`、`\pageref` 等，应按各命令的可见语义逐个设计并复用统一分类器，不能由这条正则推导所有 `*ref` 的行为。

### 7.5 `section spacing`

- 第一条规则在 `section`、`subsection`、`subsubsection` 前后增加源码空行；它会把标题前的注释行也当作正文，且不支持星号形式、可选短标题或多行标题。
- 第二条规则只删除标题后下一命令前的缩进；当前典型场景是 `\section` 后立即跟 `\subsection`，属于源码风格而非模板语义。
- 标题后人为加入空段落可能干扰首段处理，不能用源码空行控制最终版式。

归类：formatter/人工风格检查，不进入模板，也不自动改正文。

### 7.6 `displaymath`

- 第一条无 replacement 的跨行规则主要定位 `\[` 后第一行已经有公式内容的 display，供人工统一为开闭定界符独占一行；它仍可能跨过非预期内容，只能作为搜索器。
- `$$...$$ -> \[...\]` 的迁移方向正确，因为前者是 plain TeX 写法；当前候选集中在旧历年卷答案。不过 `[\w\W]` 会跨任意内容，不能排除注释、verbatim 或转义美元符号。

归类：用 ChkTeX 等 lint 报告 `$$`，再逐处迁移和编译；delimiter 的换行风格交给 formatter。

### 7.7 `matrices`

两条配套规则把 `\left|\begin{array}{...} ... \end{array}\right|` 改成 `vmatrix`，只在竖线确实表示行列式、列格式只是普通居中列时等价。开闭两侧独立替换可能产生不配对环境；opening 中的 `\{.+\}` 还可能吞掉同行后续花括号。`array` 的 `l`/`r` 对齐、列间竖线和 `@{...}` 等信息也会在转换后丢失。当前仓库没有精确命中，属于历史规则或已完成的清理。

归类：只对完整且已确认等价的一对 delimiter 做人工语义迁移，不保留为全局替换。

### 7.8 `dots`

`(,\s*)\cdots(\s*,) -> $1\ldots$2` 用于把逗号分隔列表中的居中省略号改为低省略号，符合数学排版惯例；也可考虑使用由 amsmath 根据上下文选择的 `\dots`。规则不判断数学模式，`\s*` 又可以跨行，因此仍可能进入注释、宏定义或 verbatim。

归类：这是相对高置信度但仍需逐处 review 的语义 lint；若实施，应作为独立正文清理提交并检查公式输出。

### 7.9 proof 搜索

最后的 `(?!\n) *\\begin\{proof\}` 使用了负向前瞻，检查的是“当前位置的下一个字符不是换行”，而不是“前一个字符不是换行”。引擎可以直接从 `\begin` 的反斜杠开始并让空格匹配零次，所以它实际上等价于搜索全部 proof：当前两种搜索都得到 399 个字面 occurrence，其中 3 个位于注释，实际 active proof 为 396 个。

若原意是寻找没有独占一行的 proof，应改为按行锚定、要求 proof 前已有非空白内容并排除注释的查询，例如 `(?m)^(?![ \t]*%)[ \t]*\S.*?\\begin\{proof\}`。这只是源码布局检查，与 proof/QED 的模板语义无关。

总体上，`line wrap`、`item wrap`、`dot spacing`、`section spacing`、第一条 `displaymath` 和 proof 行布局属于 lint/格式化候选；`$$`、行列式 array 和逗号列表中的 `\cdots` 属于需单独审核的语义迁移；只有引用后的中文间距属于模板层问题。现有 `patterns.re` 不应成为构建步骤或一次性批处理脚本。
