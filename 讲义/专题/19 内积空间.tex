...
在此前的章节中，我们研究线性空间时都只注重于线性结构，没有考虑其中向量的度量性质，如向量的长度、夹角等. 但度量性质恰是在分析、几何问题中不可缺少的. 故从此章起，我们引入度量的概念，将线性空间推广为内积空间.

\section{内积和范数}

\subsection{内积和范数的定义及性质}

我们先从实线性空间出发. 在 $\mathbf{R}^{2}$ 与 $\mathbf{R}^{3}$ 中，我们早已熟悉向量的点积与长度；特别地，向量长度的平方恰好等于它与自身的点积. 因此，若要在一般的实线性空间中讨论长度、夹角等度量性质，自然的想法便是先把点积的共同性质抽象出来，这便导向了\term{内积}\index{neiji@内积 (inner product)}的概念；而由内积诱导出的范数，则正是向量长度的抽象. 这与我们此前从向量空间抽象到线性空间的思路是一致的.

\begin{definition}{实内积的定义}{实内积的定义}
设 $V$ 是一个实线性空间. $V$ 上的一个实内积是一个函数，它把 $V$ 中元素的每个有序对 $(u, v)$ 都映射成一个实数 $\langle u, v \rangle \in \mathbf{R}$，并且具有以下性质：
\begin{enumerate}
    ...
    \item 第一个位置的齐性：$\forall \lambda \in \mathbf{R}, \enspace \forall u, v \in V, \enspace \langle \lambda u, v \rangle = \lambda \langle u, v \rangle$；

    \item 对称性：$\forall u, v \in V, \enspace \langle u, v \rangle = \langle v, u \rangle$.
\end{enumerate}
\end{definition}

一旦给定实内积，便可定义相应的范数：对任意 $v \in V$，令 $\lVert v \rVert = \sqrt{\langle v, v \rangle}$. 这正是欧氏空间中``长度平方等于向量与自身点积''的抽象. 例如，$\mathbf{R}^{n}$ 上的点积
\[
    (x_1, \ldots, x_n) \cdot (y_1, \ldots, y_n) = x_1 y_1 + \cdots + x_n y_n
\]
就是一个实内积（读者可以自行验证，也可以视为\autoref{ex:内积和范数}中第一部分的特例），而它诱导出的范数就是通常的欧氏长度.

接下来希望把这一思路推广到复线性空间. 然而在复空间中，若仍直接仿照实情形写成 $z_1w_1 + \cdots + z_nw_n$，那么向量与自身的``内积''未必是非负实数. 事实上，对 $z = (z_1, \ldots, z_n) \in \mathbf{C}^{n}$ 而言，向量长度平方的自然推广应是
\[
    \lvert z_1 \rvert^2 + \cdots + \lvert z_n \rvert^2 = z_1 \overline{z_1} + \cdots + z_n \overline{z_n}.
\]
这说明为了保持 $\lVert z \rVert^2 = \langle z, z \rangle$ 这一基本关系，复内积的定义中必须引入复共轭，相应地，对称性也要调整为共轭对称性. 因此，对实线性空间与复线性空间，我们可以统一给出如下定义：
...
每个实数都等于它的复共轭，所以实线性空间上的内积定义正好是上述统一定义在 $\mathbf{F} = \mathbf{R}$ 时的特例. 将线性空间与定义在其上的内积组合起来，便得到了内积空间.
...
在一般内积空间中，也同样可由该内积确定一个相应的范数：对于 $v \in V$，$v$ 的\term{范数}\index{fanshu@范数 (norm)}（记作 $ \lVert v \rVert $）定义为 $ \lVert v \rVert = \sqrt{\langle v, v \rangle}$. 并且具有以下性质：
...
\begin{proof}
    ...
    \begin{align*}
        \lVert u + v \rVert^2 & = \langle u + v, u + v \rangle \\
                              ...
                              & = \lVert u \rVert^2 + \lVert v \rVert^2 \qedhere
    \end{align*}
\end{proof}
...
\begin{proof}
    ...
    \[\lVert u + v \rVert \le \lVert u \rVert + \lVert v \rVert. \qedhere\]
\end{proof}
...
\begin{proof}
    ...
    \]
    即
    \[ \lVert a_1e_1 + \cdots + a_m e_m\rVert^2 = \lvert a_1 \rvert^2 + \cdots + \lvert a_m \rvert^2. \qedhere \]
\end{proof}
...
此外，借助 Gram-Schmidt 过程，显然我们可以得到以下结论：
\begin{enumerate}
...
Gram-Schmidt 过程还有一个重要的矩阵应用，这就是 \term{QR 分解}，其本质就是将 Gram-Schmidt 过程表达成矩阵形式. 我们先讨论方阵且列向量线性无关的情形：

\begin{theorem}{QR 分解}{QR分解}
    设 $A$ 是数域 $\mathbf{F}$ 上的 $n$ 阶可逆矩阵. 则存在唯一的 $n$ 阶矩阵 $Q$ 与 $R$，使得
    \[
        A = QR,
    \]
    其中 $Q$ 的列向量构成 $\mathbf{F}^{n}$ 的一组标准正交基，$R$ 是主对角元全为正实数的上三角矩阵.
\end{theorem}

\begin{proof}
    设 $A$ 的列向量为 $\alpha_1, \ldots, \alpha_n$. 因为 $A$ 可逆，所以 $\alpha_1, \ldots, \alpha_n$ 线性无关. 对这组向量施行 Gram-Schmidt 过程，得到 $\mathbf{F}^n$ 的一组标准正交基 $q_1, \ldots, q_n$.

    记 $u_1 = \alpha_1$, 对 $j = 2, \ldots, n$，记
    \[
        u_j = \alpha_j - \langle \alpha_j, q_1 \rangle q_1 - \cdots - \langle \alpha_j, q_{j-1} \rangle q_{j-1}.
    \]
    由 Gram-Schmidt 过程的构造，
    \[
        q_j = \frac{u_j}{\lVert u_j \rVert}, \enspace \lVert u_j \rVert > 0.
    \]
    对每个 $j = 1, \ldots, n$，定义
    \[
        r_{ij} =
        \begin{cases}
            \langle \alpha_j, q_i \rangle, & i < j, \\
            \lVert u_j \rVert,             & i = j, \\
            0,                             & i > j.
        \end{cases}
    \]
    于是由 $u_j$ 的定义可得
    \[
        \alpha_j = r_{1j} q_1 + \cdots + r_{j-1,j} q_{j-1} + u_j
        = r_{1j} q_1 + \cdots + r_{j-1,j} q_{j-1} + r_{jj} q_j.
    \]

    现在令
    \[
        Q = (q_1, \ldots, q_n), \enspace R = (r_{ij})_{n \times n}.
    \]
    由于 $Q$ 的列向量构成一组标准正交基，$R$ 又是上三角矩阵，故
    \[
        QR = (q_1, \ldots, q_n)
        \begin{pmatrix}
            ...
            0      & 0      & \cdots & r_{nn}
        \end{pmatrix}
        = (\alpha_1, \ldots, \alpha_n) = A.
    \]
    并且对每个 $j$ 都有 $r_{jj} = \lVert u_j \rVert > 0$，所以 $R$ 的主对角元全为正实数.

    下面证明唯一性. 设 $A = Q'R'$ 也是满足条件的分解，其中 $Q'$ 的列向量为 $q_1', \ldots, q_n'$，$R' = (r_{ij}')_{n \times n}$ 也是主对角元全为正实数的上三角矩阵. 对第一列有 $\alpha_1 = r_{11} q_1 = r_{11}' q_1'$，两边取范数得 $r_{11} = \lVert \alpha_1 \rVert = r_{11}'$，从而 $q_1 = q_1'$.

    设对某个 $k \geqslant 2$，已经证明 $q_1 = q_1', \ldots, q_{k-1} = q_{k-1}'$. 由矩阵乘法第 $k$ 列可得
    \[
        \alpha_k = r_{1k} q_1 + \cdots + r_{kk} q_k = r_{1k}' q_1' + \cdots + r_{kk}' q_k'.
    \]
    对于 $i < k$，因为 $q_i = q_i'$，所以
    \[
        r_{ik} = \langle \alpha_k, q_i \rangle = \langle \alpha_k, q_i' \rangle = r_{ik}'.
    \]
    从上式减去前 $k-1$ 项，得到
    \[
        \alpha_k - \sum_{i=1}^{k-1} r_{ik} q_i = r_{kk} q_k = r_{kk}' q_k'.
    \]
    再取范数，得 $r_{kk} = r_{kk}'$；由于它们都是正数，便进一步得到 $q_k = q_k'$. 由数学归纳法，$Q = Q'$ 且 $R = R'$，故分解唯一.
\end{proof}

下面给出一个 QR 分解的计算例子来帮助理解 Gram-Schmidt 过程与 QR 分解之间的联系.
...
事实上，上述证明也适用于非方阵的满列秩情形：若 $A$ 是 $m \times n$ 矩阵，且 $m \geqslant n$、$r(A) = n$，那么对它的 $n$ 个列向量施行 Gram-Schmidt 过程，便可得到
\[
    A = QR,
\]
其中 $Q$ 是 $m \times n$ 矩阵，列向量构成 $\mathbf{F}^m$ 的一个标准正交组，$R$ 是主对角元全为正实数的 $n$ 阶上三角矩阵. 若进一步将 $Q$ 的列向量扩充为 $\mathbf{F}^m$ 的一组标准正交基，就得到此前讨论的方阵形式.

若 $A$ 非列满秩，那么在 Gram-Schmidt 过程中会出现某些 $u_j = \vec{0}$，这时跳过这些零向量，只保留其中非零的正交向量，便可得到一个``较瘦''的 QR 分解，其中 $Q$ 的列向量构成 $A$ 的列空间的一组标准正交基.

在之后讨论内积空间上线性映射的矩阵表示时，我们将重点关注线性映射在某个标准正交基下的矩阵表示. 下面的 Schur 定理保证了至少存在一组标准正交基，使得线性映射在该基下的矩阵表示具有上三角形式，为后续的讨论奠定了基础. 其证明并不复杂，只需基于 Gram-Schmidt 过程和复线性空间中上三角矩阵表示的存在性和等价条件即可.
...
\begin{proof}
    由\autoref{thm:上三角矩阵存在}，存在 $V$ 的一组基 $v_1, \ldots, v_n$，使得 $T$ 关于这组基的矩阵是上三角矩阵. 根据\autoref{thm:上三角矩阵等价条件}，对每个 $j = 1, \ldots, n$ 都有
    \[
        Tv_j \in \spa(v_1, \ldots, v_j).
    \]

    对 $v_1, \ldots, v_n$ 施行 Gram-Schmidt 过程，得到 $V$ 的一组标准正交基 $e_1, \ldots, e_n$，并且对每个 $j = 1, \ldots, n$ 都有
    \[
        \spa(v_1, \ldots, v_j) = \spa(e_1, \ldots, e_j).
    \]
    于是对每个 $j = 1, \ldots, n$，
    \[
        Te_j \in T(\spa(v_1, \ldots, v_j)) \subseteq \spa(v_1, \ldots, v_j) = \spa(e_1, \ldots, e_j).
    \]
    再次利用\autoref{thm:上三角矩阵等价条件}，可知 $T$ 关于标准正交基 $e_1, \ldots, e_n$ 的矩阵是上三角矩阵.
\end{proof}
...
\begin{solution}
    \begin{enumerate}
        ...
        \]
        由\autoref{thm:Riesz}，存在唯一的 $\vec{\beta} \in V$ 使得对任意 $\vec{x} \in V$ 都有
        \[
...
\end{enumerate}
其中由 \ref*{item:23:正交投影性质:7} 我们知道正交投影一定是幂等的. 事实上，实内积空间上的正交投影还有一个非常重要的刻画：它们恰好就是对称幂等变换. 这里所谓对称，是指$\sigma\in \mathcal{L}(V)$满足$\langle \sigma(u),v\rangle = \langle u,\sigma(v)\rangle,\enspace \forall u,v\in V$. 在下一讲中，我们会知道这里对称的定义等价于 $\sigma$ 的矩阵表示是对称矩阵（\autoref{thm:伴随与共轭转置}）.

\begin{theorem}{}{对称幂等变换与正交投影的关系}
    设$V$是有限维实内积空间，$\sigma\in \mathcal{L}(V)$. 则$\sigma$是正交投影，当且仅当$\sigma$是对称幂等变换.
\end{theorem}

\begin{proof}
    必要性：若$\sigma=P_U$是到子空间$U$上的正交投影，则由上面的性质知$\sigma^2=\sigma$. 对任意$u,v\in V$，设$u=u_1+u_2,\enspace v=v_1+v_2$，其中$u_1,v_1\in U,\enspace u_2,v_2\in U^\perp$，则$\sigma(u)=u_1,\sigma(v)=v_1$. 因此$\langle \sigma(u),v\rangle=\langle u_1,v_1+v_2\rangle=\langle u_1,v_1\rangle$，而$\langle u,\sigma(v)\rangle=\langle u_1+u_2,v_1\rangle=\langle u_1,v_1\rangle$，故$\langle \sigma(u),v\rangle=\langle u,\sigma(v)\rangle$，于是$\sigma$是对称的.

    充分性：若$\sigma^2=\sigma$，根据\autoref{thm:幂等变换等价于投影变换}可知 $V = \im \sigma \oplus \ker \sigma$ 以及 $\sigma$ 是 $V$ 到 $\im \sigma$ 上的投影变换，故只需证明 $\ker \sigma = (\im \sigma)^\perp$. 记 $U = \im \sigma$，若 $x\in\ker \sigma$，则对任意 $y\in V$，因为 $\sigma(y)\in U$ 且 $\sigma$ 是对称的，故$\langle x,\sigma(y)\rangle=\langle \sigma(x),y\rangle=0$，从而$x\in U^\perp$. 反之，若$x\in U^\perp$，则对任意$y\in V$都有$\langle \sigma(x),y\rangle=\langle x,\sigma(y)\rangle=0$，特别地取$y=\sigma(x)$，便有$\lVert \sigma(x)\rVert^2=0$，故$\sigma(x)=0$，即$x\in\ker \sigma$. 因此$\ker \sigma=U^\perp$，故$\sigma$是正交投影变换.
\end{proof}
