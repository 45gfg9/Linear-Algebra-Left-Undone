...
\end{theorem}
\begin{proof}
    设$V$已经写成第二有理标准形对应的循环子空间直和. 对于每一个以$p$为底的初等因子$p^{m_j}$，对应的循环子空间同构于$\mathbf{F}[x]/(p^{m_j})$. 在这个商空间上，$p(T)$对应于乘以$p(x)$这一算子，因此对任意$i\geqslant 1$，
    \[
...
\begin{proof}
    ...
    由初等因子组到不变因子组的确定方式，就是\autoref{eq:20:不变因子分解} 前面的按底数分行、按幂次对齐、按列相乘的过程，这一过程显然是唯一的.
