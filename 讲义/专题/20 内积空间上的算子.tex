...
\begin{definition}{}{}
    设 $ V $ 和 $ U $ 是数域 $ \mathbf{F} $ 上的两个内积空间. $ S: V \rightarrow U $ 是一个线性映射. 若 $ \forall u, v \in V,\enspace \langle Su, Sv \rangle_U = \langle u, v \rangle_V $，则称 $ S $ 是一个保持内积的线性映射. 若 $ S $ 是双射，则称 $ S $ 是一个\term{保积同构}.
\end{definition}

在内积空间的语境下，保积同构也被称作同构. 显然，因为其能够保持内积，所以也能够保持范数. 那么其逆命题能否成立？很幸运，答案是肯定的.
...
\begin{proof}
    由于我们的条件是范数的保持，结论是内积的保持，所以这里需要用范数来表示内积，回想起\autoref{eq:23:内积和范数的性质1} 与\autoref{eq:23:内积和范数的性质3}，我们有
    \begin{enumerate}
        \item $\mathbf{F}=\mathbf{R}$时
        \begin{align*}
            \langle Su, Sv \rangle_U & = \frac{\lVert Su + Sv \rVert_U^2 - \lVert Su - Sv \rVert_U^2}{4} \\
                                     & = \frac{\lVert u + v \rVert_V^2 - \lVert u - v \rVert_V^2}{4}     \\
                                     & = \langle u, v \rangle_V.
        \end{align*}
        \item $\mathbf{F}=\mathbf{C}$时
        \begin{align*}
            \langle Su, Sv \rangle_U & = \frac{\lVert Su + Sv \rVert_U^2 - \lVert Su - Sv \rVert_U^2 + \i(\lVert Su + \i Sv \rVert_U^2 - \lVert Su - \i Sv \rVert_U^2)}{4} \\
                                     & = \frac{\lVert u + v \rVert_V^2 - \lVert u - v \rVert_V^2 + \i(\lVert u + \i v \rVert_V^2 - \lVert u - \i v \rVert_V^2)}{4}     \\
                                     & = \langle u, v \rangle_V.
        \end{align*}
    \end{enumerate}
    从而总有 $ \langle Su, Sv \rangle_U = \langle u, v \rangle_V $，所以 $ S $ 保持内积.
\end{proof}
...
\begin{theorem}{}{}
    ...
    \begin{enumerate}[label=(\arabic*)]
        \item \label{item:20:保积同构条件:1}
            $ S $ 保持内积.
        \item \label{item:20:保积同构条件:2}
            $ S $ 是保积同构.
        \item \label{item:20:保积同构条件:3}
            $ S $ 将 $ V $ 的任一组标准正交基映射到 $ U $ 的一组标准正交基.
        \item \label{item:20:保积同构条件:4}
            $ S $ 将 $ V $ 的某一组标准正交基映射到 $ U $ 的一组标准正交基.
    \end{enumerate}
\end{theorem}

\begin{proof}
    \begin{itemize}
        \item[\ref*{item:20:保积同构条件:1}$\implies$\ref*{item:20:保积同构条件:2}] 由内积的保持可知范数的保持，从而对于任意 $v\in V$，$ Sv=\vec{0} \iff \|Sv\|=\|v\|=\vec{0} \iff v=\vec{0}$，所以 $ S $ 是单射. 而 $ V $ 和 $ U $ 维数相同，进而是双射.

        \item[\ref*{item:20:保积同构条件:2}$\implies$\ref*{item:20:保积同构条件:3}] 设 $ e_1, \ldots , e_n $ 是 $ V $ 的任意一组标准正交基，则
        \[ \langle Se_i, Se_j \rangle_U = \langle e_i, e_j \rangle_V = \delta_{ij},\enspace i, j = 1, \ldots , n. \]
        所以 $ Se_1, \ldots , Se_n $ 是 $ U $ 的一组标准正交基.

        \item[\ref*{item:20:保积同构条件:3}$\implies$\ref*{item:20:保积同构条件:4}] 显然.

        \item[\ref*{item:20:保积同构条件:4}$\implies$\ref*{item:20:保积同构条件:1}] 设 $ e_1, \ldots , e_n $ 是 $ V $ 的一组标准正交基，$ f_1, \ldots , f_n $ 是 $ U $ 的一组标准正交基，且 $ Se_i = f_i,\enspace i = 1, \ldots , n $. 对任意的 $ u, v \in V $, 存在唯一的一组标量 $ a_1, \ldots , a_n, b_1, \ldots , b_n \in \mathbf{F} $, 使得
        \[ u = a_1e_1 + a_2e_2 + \cdots + a_ne_n, \enspace v = b_1e_1 + b_2e_2 + \cdots + b_ne_n. \]
        则
        \begin{align*}
            \langle Su, Sv \rangle_U & = \left\langle S\left(\sum_{i=1}^{n} a_ie_i\right), S\left(\sum_{j=1}^{n} b_je_j\right) \right\rangle_U    \\
                                & = \left\langle \sum_{i=1}^{n} a_if_i, \sum_{j=1}^{n} b_jf_j \right\rangle_U   \\
                                & = \sum_{i=1}^{n} a_i\overline{b_i} \langle f_i, f_i \rangle_U + \sum_{i \neq j} a_i\overline{b_j} \langle f_i, f_j \rangle_U \\
                                & = \sum_{i=1}^{n} a_i\overline{b_i} \langle e_i, e_i \rangle_V + \sum_{i \neq j} a_i\overline{b_j} \langle e_i, e_j \rangle_V \\
                                & = \langle u, v \rangle_V.
        \end{align*}
        故 $ S $ 保持内积.
    \end{itemize}
...
由于同构只与维数有关，为了研究的简便，我们不妨研究同一线性空间下的保积同构，也就是线性变换. 但在此之前，我们需要讲明线性映射在内积作用下的对称性问题.
...
利用伴随映射的定义不难得到伴随的如下基本性质：

\begin{example}{伴随的运算性质}{}
\begin{enumerate}
...
\begin{proof}
    前 4 条性质均不难验证. 这里仅证明第 5 条性质，事实上也非常简单：

    对于任意的 $ v \in V, w \in W $，有
    \[
        \langle (ST)v, w \rangle = \langle S(Tv), w \rangle = \langle Tv, S^*w \rangle = \langle v, T^*S^*w \rangle,
    \]
    因此 $ (ST)^* = T^*S^* $.
\end{proof}
...
这种操作的地位近似于实空间中的转置，与复空间上内积的共轭对称性相关联. 伴随映射与原映射在对应标准正交基下的表示矩阵正是这种对称关系（该对称关系在实内积空间下变为转置）.

\begin{theorem}{}{伴随与共轭转置}
...
从伴随映射与原映射的核空间、像空间之间的关系可以看出伴随与原映射之间的对称性，具体来说：

\begin{theorem}{}{伴随核空间像空间}
...
\begin{proof}
    仅证明第 1, 2 条性质，利用 $(T^*)^*=T$ 可得到 3, 4 的正确性.
    \begin{enumerate}
        \item 设 $ w \in \ker T^* $，则 $ T^*w = 0 $，所以 $ \forall v \in V, \langle Tv, w \rangle = \langle v, T^*w \rangle = 0 $，即 $ w \in (\im T)^{\perp} $，所以 $ \ker T^* \subset (\im T)^{\perp} $.

        设 $ w \in (\im T)^{\perp} $，则 $ \forall v \in V, \langle Tv, w \rangle = 0 $，所以 $ \forall v \in V, \langle v, T^*w \rangle = 0 $，故 $ T^*w = 0 $，即 $ w \in \ker T^* $，从而 $ (\im T)^{\perp} \subset \ker T^* $.

        因此，$ \ker T^* = (\im T)^{\perp} $.

        \item 设 $ v \in \im T^* $，则存在 $ w \in W $ 使得 $ v = T^*w $，所以 $ \forall u \in \ker T, \langle u, v \rangle = \langle u, T^*w \rangle = \langle Tu, w \rangle = 0 $，即 $ v \in (\ker T)^{\perp} $，因此 $ \im T^* \subset (\ker T)^{\perp} $.

        设 $ v \in (\ker T)^{\perp} $，则 $ \forall u \in \ker T, \langle u, v \rangle = 0 $，考虑线性泛函 $ \varphi: \im T \to \mathbf{F} $ 定义为 $ \varphi(Tu) = \langle u, v \rangle $，该定义良好且线性，由 Riesz 表示定理存在 $ w \in W $ 使得 $ \varphi(y) = \langle y, w \rangle $，于是 $ \langle u, v \rangle = \langle Tu, w \rangle = \langle u, T^*w \rangle $，故 $ v = T^*w $，即 $ v \in \im T^* $，因此 $ (\ker T)^{\perp} \subset \im T^* $.

        因此，$ \im T^* = (\ker T)^{\perp} $.
    \end{enumerate}
\end{proof}

这一对称性同样寓于对应的特征值、不变子空间中：
...
\begin{proof}
    由定义可知，$ \lambda $ 是 $ T $ 的特征值当且仅当 $ T - \lambda I $ 不是单射，这又等价于 $ T - \lambda I $ 不是满射. 根据\autoref{thm:伴随核空间像空间}，这等价于
    \[
        \ker \bigl( (T - \lambda I)^* \bigr) \neq \{ 0 \}.
    \]
    而
    \[
        (T - \lambda I)^* = T^* - \overline{\lambda} I,
    \]
    因而这又等价于 $ T^* - \overline{\lambda} I $ 不是单射，也即 $ \overline{\lambda} $ 是 $ T^* $ 的特征值.
\end{proof}
...
\begin{proof}
$U$ 在 $T$ 下不变当且仅当对任意的 $u\in U$，$Tu \in U$. 由此可知对任意的 $v\in U^{\perp}$，$\langle Tu, v \rangle = \langle u, T^*v \rangle = 0$，所以 $T^*v \in U^{\perp}$，即 $U^{\perp}$ 在 $T^*$ 下不变. 反之亦然.
\end{proof}
...
\begin{proof}
    设 $ e_1, \cdots, e_n $ 是 $ V $ 的一组标准正交基，$ A = (a_{ij}) $ 是 $ S $ 在这组基下的表示矩阵. 则对每个 $ j = 1, \cdots, n $，有
    \[
        S e_j = a_{1j} e_1 + \cdots + a_{nj} e_n.
    \]
    由于 $ S $ 是酉变换（正交变换），$ S e_1, \cdots, S e_n $ 仍构成一组标准正交基. 因此对任意 $ j, k $，有
    \[
        \delta_{jk} = \langle S e_j, S e_k \rangle
        = \sum_{i = 1}^n a_{ij} \overline{a_{ik}}.
    \]
    这说明 $ A^\mathrm{H} A = E $（实数情形下为 $ A^{\mathrm{T}} A = E $），故 $ A $ 是酉矩阵（正交矩阵）.
\end{proof}
...
酉矩阵和正交矩阵与标准正交基的关联远不止于表示矩阵，事实上，它们是标准正交基之间的桥梁，也就是过渡矩阵.
...
为了研究自伴算子的酉相似（正交相似）标准型，首先得了解自伴算子的特征值和不变子空间的情况. 回忆伴随部分的\autoref{ex:伴随与特征值}，我们不难得到自伴算子的特征值的性质.
...
虽然我们阐明了自伴算子的特征值不与复数纠缠在一块，但还有另外一个阴影，就是它会不会像正交矩阵对应的算子那样，在实数域上没有特征值，这也是我们不希望看到的一点. 幸运的是，可以证明实特征值的存在性，这需要一点技巧，我们先从一个引理开始.
...
对于该引理的证明，第一条参考\autoref{ex:伴随与不变子空间}，第二条和第三条直接利用自伴算子的定义即可.

该引理表明，自伴算子在不变子空间上的限制还是自伴算子. 这样的一条性质给归纳法提供了一个很好的切入点，利用它可以给出自伴算子的完全描述，我们称之为实谱定理.
...
\begin{proof}
    采取 3 $\implies$ 1 $\implies$ 2 $\implies$ 3 进行证明.
    ...
    \begin{itemize}
        ...
        \item[\ref*{item:24:实谱定理:1}$\implies$\ref*{item:24:实谱定理:2}] 采用数学归纳法.
              ...
              设 $ \dim V > 1 $ 且在维数更小的实内积空间上成立. 因为\autoref{lem:实谱定理引理2}，设 $ T $ 有一个特征向量 $ u $ 且 $ \lVert u \rVert = 1 $，$ U = \spa(u) $，则 $ U $ 是 $ V $ 的一个一维子空间且在 $ T $ 下不变，由\autoref{lem:实谱定理引理3}，算子 $ T|_{U^{\perp }} \in \mathcal{L}(U^{\perp }) $ 是自伴的.

              由归纳假设，$ U^{\perp } $ 有一组由 $ T|_{U^{\perp }} $ 的特征向量构成的标准正交基. 将 $ u $ 添加进这组基，就得到了 $ V $ 的一组由 $ T $ 的特征向量构成的标准正交基，得证.
...
正规算子在标准正交基下的表示矩阵便称为\term{正规矩阵}，在实内积空间下满足 $ AA^{\mathrm{T}} = A^{\mathrm{T}}A $，复内积空间下满足 $ AA^{\mathrm{H}} = A^{\mathrm{H}}A $. 不难发现，其实自伴算子、酉算子、正交算子都是特殊的正规算子，正规算子将我们先前研究的算子都囊括了进来.
...
\begin{proof}
    设 $ T $ 是正规的，则 $ TT^* = T^*T $，对任意 $ v \in V $ 有
    \[
    \lVert Tv \rVert^2 = \langle Tv, Tv \rangle = \langle T^*Tv, v \rangle = \langle TT^*v, v \rangle = \langle T^*v, T^*v \rangle = \lVert T^*v \rVert^2.
    \]

    反之，设 $ \forall v \in V,\enspace \lVert Tv \rVert = \lVert T^*v \rVert $，则
    \begin{align*}
        0 & = \lVert Tv \rVert^2 - \lVert T^*v \rVert^2               \\
          & = \langle Tv, Tv \rangle - \langle T^*v, T^*v \rangle     \\
          & = \langle T^*Tv, v \rangle - \langle TT^*v, v \rangle     \\
          & = \langle (T^*T - TT^*)v, v \rangle.
    \end{align*}
    记 $ S = T^*T - TT^* $，则 $S$ 是自伴的，由\autoref{thm:实谱定理} 可知 $V$ 可拆成 $S$ 的若干特征子空间 $U_1,\ldots,U_m$ 的直和，对于任意的 $u \in U_i$，$\langle Su, u \rangle = 0$，故 $Su \in U_i^{\perp}$，但 $U_i$ 在 $S$ 下不变，故 $Su \in U_i \cap U_i^{\perp} = \{\vec{0}\}$，所以 $S\vert_{U_i} = 0$，$ i = 1, \ldots, m $，从而 $ S = 0 $，即 $ TT^* = T^*T $，$ T $ 是正规的.
\end{proof}
...
\begin{proof}
    令 $ N = T - \lambda I $，则 $N^* = T^* - \overline{\lambda} I$. 由 $ T $ 是正规的可得
    \begin{align*}
        N^* N &= (T^* - \overline{\lambda} I)(T - \lambda I) \\
        &= T^* T - \lambda T^* - \overline{\lambda} T + |\lambda|^2 I \\
        &= T T^* - \lambda T^* - \overline{\lambda} T + |\lambda|^2 I \\
        &= (T - \lambda I)(T^* - \overline{\lambda} I) = N N^*.
    \end{align*}
    因而 $ N $ 也是正规的. 由\autoref{thm:正规算子的等价条件} 的条件 2 可知，对任意 $ x \in V $ 都有 $ \| N x \| = \| N^* x \| $. 由于 $ v $ 是 $ T $ 相应于 $ \lambda $ 的特征向量，故
    \[
        N v = (T - \lambda I)v = 0.
    \]
    从而
    ...
    因此 $ N^* v = 0 $，即
    \[
        (T^* - \overline{\lambda} I)v = 0.
    \]
    这说明 $ v $ 是 $ T^* $ 相应于特征值 $ \overline{\lambda} $ 的特征向量.
\end{proof}
...
% TODO: 复正规矩阵正交对角化+实正规矩阵正交分块对角化 习题
...
\begin{proof}
    对 $ \dim V $ 作归纳. 当 $ \dim V = 1 $ 时结论显然成立.

    设结论对维数小于 $ \dim V $ 的情形成立. 由代数学基本定理，$ T $ 至少有一个特征值 $ \lambda $. 取相应的单位特征向量 $ e_1 $，令 $ U = \spa(e_1) $. 任取 $ v \in U^\perp $，由\autoref{thm:正规算子的特征向量} 可知
    \[
        T^* e_1 = \overline{\lambda} e_1.
    \]
    因而
    \[
        \langle Tv, e_1 \rangle = \langle v, T^* e_1 \rangle = \langle v, \overline{\lambda} e_1 \rangle = \lambda \langle v, e_1 \rangle = 0.
    \]
    这说明 $ U^\perp $ 在 $ T $ 下不变. 另一方面，由\autoref{ex:伴随与不变子空间} 可知 $ U^\perp $ 在 $ T^* $ 下也不变. 因而 $ T|_{U^\perp} \in \mathcal{L}(U^\perp) $. 对任意 $ x, y \in U^\perp $，有
    \[
        \langle T|_{U^\perp} x, y \rangle = \langle Tx, y \rangle = \langle x, T^* y \rangle = \langle x, T^*|_{U^\perp} y \rangle,
    \]
    故 $ (T|_{U^\perp})^* = T^*|_{U^\perp} $. 于是
    \[
        T|_{U^\perp}(T|_{U^\perp})^* = T|_{U^\perp} T^*|_{U^\perp} = (TT^*)|_{U^\perp} = (T^*T)|_{U^\perp} = T^*|_{U^\perp} T|_{U^\perp} = (T|_{U^\perp})^* T|_{U^\perp},
    \]
    所以 $ T|_{U^\perp} $ 也是正规的.

    由归纳假设，$ U^\perp $ 有一组由 $ T|_{U^\perp} $ 的特征向量构成的标准正交基 $ e_2, \cdots, e_n $. 再加上 $ e_1 $，便得到 $ V $ 的一组由 $ T $ 的特征向量构成的标准正交基. 因此复谱定理成立.
\end{proof}
...
\begin{proof}
    若 $ T $ 是自伴的，则由前面已经证明过的结论可知 $ T $ 的所有特征值都是实数.

    反之，设 $ T $ 是正规的，且它的所有特征值都是实数. 由复谱定理，$ V $ 有一组由 $ T $ 的特征向量构成的标准正交基 $ e_1, \cdots, e_n $. 设
    \[
        T e_j = \lambda_j e_j, \quad j = 1, \cdots, n,
    \]
    其中 $ \lambda_1, \cdots, \lambda_n \in \mathbb{R} $. 因而 $ T $ 在这组标准正交基下的表示矩阵为 $\diag(\lambda_1, \cdots, \lambda_n)$. 由\autoref{thm:伴随与共轭转置} 可知，$ T^* $ 在同一组标准正交基下的表示矩阵是该矩阵的共轭转置，仍等于它自身，故 $ T^* = T $，即 $ T $ 是自伴算子.
\end{proof}
...
\begin{proof}
    (1) $ \Rightarrow $ (2) 设 $ S $ 是酉变换. 由前面的讨论可知 $ S^{-1} = S^* $，于是
    \[
        S^* S = S S^* = I,
    \]
    从而 $ S $ 是正规的. 由复谱定理，$ V $ 有一组由 $ S $ 的特征向量构成的标准正交基. 若 $ v $ 是 $ S $ 相应于特征值 $ \lambda $ 的特征向量，则
    \[
        \| v \| = \| Sv \| = \| \lambda v \| = |\lambda| \, \| v \|.
    \]
    由于 $ v \neq 0 $，故 $ |\lambda| = 1 $.

    (2) $ \Rightarrow $ (1) 设 $ e_1, \cdots, e_n $ 是一组由 $ S $ 的特征向量构成的标准正交基，且
    \[
        S e_j = \lambda_j e_j, \quad |\lambda_j| = 1, \quad j = 1, \cdots, n.
    \]
    任取 $v = a_1 e_1 + \cdots + a_n e_n \in V$，有
    \[
        Sv = a_1 \lambda_1 e_1 + \cdots + a_n \lambda_n e_n.
    \]
    因而
    \[
        \| Sv \|^2 = |a_1 \lambda_1|^2 + \cdots + |a_n \lambda_n|^2 = |a_1|^2 + \cdots + |a_n|^2 = \| v \|^2.
    \]
    任取
    \[
        u = b_1 e_1 + \cdots + b_n e_n \in V,
    \]
    则
    \[
        \langle Sv, Su \rangle = \sum_{j = 1}^n a_j \overline{b_j} |\lambda_j|^2 = \sum_{j = 1}^n a_j \overline{b_j} = \langle v, u \rangle.
    \]
    因而 $ S $ 保持内积，所以 $ S $ 是酉变换.
\end{proof}
...
证明是非常容易的：只需要利用矩阵正规但不自伴的定义（$ A A^\mathrm{T} = A^\mathrm{T} A $ 且 $ A \neq A^\mathrm{T} $）就可以得到矩阵的形式了，这里不再展开.

事实上，这一定理正与我们的目标相一致. 而考虑到实谱定理的证明方法，我们希望能够得到正规算子与其不变子空间之间的联系，最重要的是在不变子空间上的限制依然是正规的，来让我们继续使用归纳法. 事实上这是可行的，我们有如下的结论.

\begin{theorem}{}{正规算子的不变子空间}
...
\begin{proof}
    设 $ \dim U = r $，取 $ U $ 的一组标准正交基 $ e_1, \cdots, e_r $，再将其扩充为 $ V $ 的一组标准正交基
    \[
        e_1, \cdots, e_r, e_{r+1}, \cdots, e_n,
    \]
    其中 $ e_{r+1}, \cdots, e_n $ 是 $ U^\perp $ 的一组标准正交基. 由于 $ U $ 在 $ T $ 下不变，$ T $ 在这组基下的矩阵必有分块上三角形式
    \[
        A = \begin{pmatrix}
            A_1 & B \\
            0   & A_2
        \end{pmatrix}.
    \]
    由\autoref{thm:伴随与共轭转置}，$ T^* $ 在这组基下的矩阵为
    \[
        A^\mathrm{H} = \begin{pmatrix}
            A_1^\mathrm{H} & 0 \\
            B^\mathrm{H}   & A_2^\mathrm{H}
        \end{pmatrix}.
    \]
    因为 $ T $ 正规，所以 $ A $ 也是正规矩阵，即 $ A A^\mathrm{H} = A^\mathrm{H} A $. 比较两边左上角的分块可得
    \[
        A_1 A_1^\mathrm{H} + B B^\mathrm{H} = A_1^\mathrm{H} A_1.
    \]
    对上式取迹，得到 $\operatorname{tr}(B B^\mathrm{H}) = 0$. 但 $ B B^\mathrm{H} $ 的对角元之和等于 $ B $ 的各个元素模平方之和，因此 $ B = 0 $. 于是 $ A $ 实际上是分块对角矩阵
    \[
        A = \begin{pmatrix}
            A_1 & 0 \\
            0   & A_2
        \end{pmatrix}.
    \]
    这立刻说明 $ U^\perp $ 在 $ T $ 下不变，从而第 1 条成立；同时 $ A^\mathrm{H} $ 也是分块对角矩阵，因此 $ U $ 在 $ T^* $ 下不变，第 2 条也成立.

    对任意 $ u, v \in U $，由第 2 条知 $ T^*v \in U $，于是
    \[
        \langle T|_U u, v \rangle = \langle Tu, v \rangle = \langle u, T^*v \rangle = \langle u, T^*|_U v \rangle.
    \]
    故 $ (T|_U)^* = T^*|_U $，第 3 条成立.

    由 $ A $ 的分块对角形式可知
    \[
        A A^\mathrm{H} = \begin{pmatrix}
            A_1 A_1^\mathrm{H} & 0 \\
            0                  & A_2 A_2^\mathrm{H}
        \end{pmatrix},
        \qquad
    ...
    由于这两个矩阵相等，所以
    \[
        A_1 A_1^\mathrm{H} = A_1^\mathrm{H} A_1, \qquad A_2 A_2^\mathrm{H} = A_2^\mathrm{H} A_2.
    \]
    这说明 $ T|_U $ 和 $ T|_{U^\perp} $ 都是正规的，第 4 条成立.
\end{proof}

接下来就只需要将整个空间分解为不变子空间的直和，就可以得到实内积空间上正规算子的完全描述了.

\begin{theorem}{}{实正规算子的结构}
...
\begin{proof}
    \begin{itemize}
        \item[(2) $ \Rightarrow $ (1)] 对角线上的每个 $ 1 \times 1 $ 块显然都是正规的，而每个
        \[
            \begin{pmatrix}
                a & -b \\
                b & a
            \end{pmatrix}
        \]
        型的 $ 2 \times 2 $ 块由\autoref{thm:二维实正规算子}也知道是正规的. 分块对角矩阵正规当且仅当每个对角块正规，因此 $ T $ 是正规的.

        \item[(1) $ \Rightarrow $ (2)] 对 $ \dim V $ 作归纳. 当 $ \dim V = 1 $ 时结论显然成立.

        设结论对维数小于 $ \dim V $ 的实内积空间成立，且 $ T $ 是正规的. 若 $ T $ 是自伴的，则由实谱定理可知 $ T $ 在某组标准正交基下是对角矩阵，结论成立.

        下面设 $ T $ 不是自伴的. 任取 $ V $ 的一组标准正交基，设 $ A $ 是 $ T $ 在这组基下的实矩阵. 把 $ A $ 看成复矩阵，则它仍是正规矩阵. 由复谱定理，$ A $ 有一个复特征值 $ \lambda $.

        若 $ \lambda \in \mathbb{R} $，取相应的复特征向量 $ z = x + \i y $，其中 $ x, y \in \mathbb{R}^n $. 由 $A z = \lambda z$ 可得
        \[
            A x = \lambda x, \qquad A y = \lambda y.
        \]
        因为 $ z \neq 0 $，故 $ x, y $ 不全为零，于是可取一个非零实向量 $ u $ 满足 $ A u = \lambda u $. 这对应于 $ V $ 中的一个实特征向量. 令 $ U = \spa(u) $，则 $ U $ 在 $ T $ 下不变. 由\autoref{thm:正规算子的不变子空间}，$ U^\perp $ 在 $ T $ 下不变，且 $ T|_{U^\perp} $ 仍是正规的. 由归纳假设，$ U^\perp $ 有一组满足条件的标准正交基；再加上 $ u $ 标准化后得到的向量，便得到 $ V $ 的一组满足条件的标准正交基.

        若 $ \lambda \notin \mathbb{R} $，则可设 $ \lambda = a + b\i $，其中 $ b > 0 $. 取 $ \lambda = a + b\i $ 的一个复特征向量 $ z = x + \i y $，其中 $ x, y \in \mathbb{R}^n $. 由 $ A z = \lambda z $ 比较实部与虚部可得
        \[
            A x = a x - b y, \qquad A y = b x + a y.
        \]
        因而 $ W = \spa(x, y) $ 在 $ T $ 下不变. 注意 $ x, y $ 线性无关；否则若 $ y = c x $，则 $ z = (1 + c\i)x $，从而 $ A x = \lambda x $，这与 $ A x $ 为实向量而 $ \lambda \notin \mathbb{R} $ 矛盾. 因此 $ \dim W = 2 $.

        由\autoref{thm:正规算子的不变子空间}，$ T|_W $ 也是正规的，且 $ W^\perp $ 在 $ T $ 下不变，$ T|_{W^\perp} $ 仍是正规的. 在基 $\{x, y\}$ 下，$ T|_W $ 的矩阵为
        \[
            \begin{pmatrix}
                a & b \\
                -b & a
            \end{pmatrix},
        \]
        因而它的特征多项式为 $(t - a)^2 + b^2$，所以 $ T|_W $ 的特征值为 $ a \pm b\i $. 特别地，$ T|_W $ 不是自伴的；否则它的特征值都应是实数. 于是 $ T|_W $ 是二维实内积空间 $ W $ 上正规的但不是自伴的算子，由\autoref{thm:二维实正规算子}，$ W $ 有一组标准正交基，使得 $ T|_W $ 在这组基下的矩阵是
        \[
            \begin{pmatrix}
                a & -b \\
                b & a
...
\begin{proof}
    \begin{itemize}
        \item[(1) $ \Rightarrow $ (2)] 设 $ S $ 是正交变换，则 $ S $ 是正规的. 由\autoref{thm:实正规算子的结构}，存在 $ V $ 的一组标准正交基，使得 $ S $ 关于这组基的矩阵是分块对角矩阵，每个块要么是 $ 1 \times 1 $ 矩阵，要么是
        \[
            \begin{pmatrix}
                a & -b \\
                b & a
            ...
            \qquad b > 0.
        \]
        对于 $ 1 \times 1 $ 块，因其对应的算子仍是正交变换，故该块必为 $ 1 $ 或 $ -1 $.

        对于 $ 2 \times 2 $ 块，设其为 $ B = \begin{pmatrix} a & -b \\ b & a \end{pmatrix} $. 由于 $ S $ 是正交变换，$ B $ 是正交矩阵，从而
        \[
            B^{\mathrm{T}} B = \begin{pmatrix}
                a^2 + b^2 & 0 \\
                0         & a^2 + b^2
            \end{pmatrix} = I.
        \]
        因此 $ a^2 + b^2 = 1 $. 又因为 $ b > 0 $，故存在唯一的 $ \theta \in (0, \pi) $ 使得
        \[
        ...
        \item[(2) $ \Rightarrow $ (1)] 题设中的每个 $ 1 \times 1 $ 块 $ 1 $ 和 $ -1 $ 都是正交矩阵，每个
        \[
            \begin{pmatrix}
                \cos \theta & -\sin \theta \\
                \sin \theta & \cos \theta
            \end{pmatrix}
        \]
        也是正交矩阵. 因而整个分块对角矩阵是正交矩阵，故 $ S $ 是正交变换.
    \end{itemize}
\end{proof}
...
因为我们先前已经给出过自伴算子的完全描述，所以正算子在对角化方面的行为与自伴算子是一致的. 下面的刻画更多是从算子与数的类比角度出发. 其中\ref*{item:25:正算子刻画:3} 就相当于复数非负当且仅当其有非负的平方根，\ref*{item:25:正算子刻画:4} 就相当于复数非负当且仅当其有实的平方根，\ref*{item:25:正算子刻画:5} 就相当于复数 $ z $ 非负当且仅当存在复数 $ w $ 使得 $ z = \overline{w}w $.
...
\begin{proof}
    \begin{itemize}
        \item[\ref*{item:25:正算子刻画:1}$\Rightarrow$\ref*{item:25:正算子刻画:2}] 设 $ T $ 是正的. 由定义，$ T $ 是自伴的. 若 $ v \neq 0 $ 是 $ T $ 相应于特征值 $ \lambda $ 的特征向量，则
        \[
            \lambda \| v \|^2 = \langle \lambda v, v \rangle = \langle Tv, v \rangle \geqslant 0.
        \]
        因而 $ \lambda \geqslant 0 $.

        \item[\ref*{item:25:正算子刻画:2}$\Rightarrow$\ref*{item:25:正算子刻画:3}] 设 $ T $ 是自伴的，且所有特征值都非负. 由前面对自伴算子的讨论可知，$ V $ 有一组由 $ T $ 的特征向量构成的标准正交基 $ e_1, \cdots, e_n $. 设
        \[
            T e_j = \lambda_j e_j, \qquad \lambda_j \geqslant 0, \quad j = 1, \cdots, n.
        \]
        定义算子 $ R \in \mathcal{L}(V) $ 为
        \[
            R e_j = \sqrt{\lambda_j} e_j, \qquad j = 1, \cdots, n.
        \]
        则 $ R $ 是自伴的，且其特征值都非负，故 $ R $ 是正的. 同时
        \[
            R^2 e_j = \lambda_j e_j = T e_j, \qquad j = 1, \cdots, n.
        \]
        因为 $ e_1, \cdots, e_n $ 是一组基，所以 $ R^2 = T $. 因而 $ R $ 是 $ T $ 的正平方根.

        \item[\ref*{item:25:正算子刻画:3}$\Rightarrow$\ref*{item:25:正算子刻画:4}] 显然，因为正算子必为自伴算子.

        \item[\ref*{item:25:正算子刻画:4}$\Rightarrow$\ref*{item:25:正算子刻画:5}] 设 $ R $ 是 $ T $ 的一个自伴平方根，则 $T = R^2 = R^* R$.

        \item[\ref*{item:25:正算子刻画:5}$\Rightarrow$\ref*{item:25:正算子刻画:1}] 设 $ T = R^* R $. 则 $T^* = (R^* R)^* = R^* R = T$，所以 $ T $ 是自伴的. 又对任意 $ v \in V $，有
        \[
            \langle Tv, v \rangle = \langle R^* Rv, v \rangle = \langle Rv, Rv \rangle = \| Rv \|^2 \geqslant 0.
        \]
        因此 $ T $ 是正的.
    \end{itemize}
\end{proof}
...
\subsection{奇异值分解的存在性与计算}

此前我们着重介绍了内积空间上的一些特殊的算子，以及它们的约化分解. 本讲我们将尝试将所得到的结论推广到一般的线性映射上，并给出一个将一般线性映射都``对角化''的方式. 在若当标准型的时候我们已经知道，这理应是做不到的，症结在于可能没有足够的特征向量. 另一方面，一般的线性映射出发空间和到达空间的维数并不一定相等，所以我们此处的``对角化''并不是使用线性映射本身的特征值和特征向量，而是采取与之相关的特殊的算子的特征值和特征向量. 回忆在对称矩阵部分，我们从一般的矩阵 $A$ 构造出了 $A^{\mathrm{T}}A$ 这样一个对称矩阵. 那么采取类似的方法，我们可以从 $T \in \mathcal{L}(V, W)$ 构造出算子 $T^*T$，让我们来看看它的性质.
...
\begin{proof}
    \begin{enumerate}
        \item 由定义知，$0$ 是 $T$ 的奇异值当且仅当 $0$ 是 $T^*T$ 的特征值. 对于正算子而言，这又等价于 $\ker T^*T \neq \{\vec{0}\}$. 由\autoref{thm:T^*T的性质} 第 2 条可知 $\ker T^*T = \ker T$，故 $0$ 不是 $T$ 的奇异值当且仅当 $\ker T = \{\vec{0}\}$，也即 $T$ 是单射.

        \item 设 $\dim V = n$. 则 $T^*T$ 的 $0$ 特征值的重数为 $\dim \ker T^*T = \dim \ker T$. 因而正奇异值的个数为 $n - \dim \ker T = \dim \im T$，最后一步由线性映射基本定理得到.

        \item $T$ 满射当且仅当 $\dim \im T = \dim W$. 再结合上述第 2 条，便得到结论.
    \end{enumerate}
\end{proof}
...
\begin{theorem}{奇异值分解}{奇异值分解定理} \index{qiyizhi!fenjie@奇异值分解 (singular value decomposition)}
    设 $T \in \mathcal{L}(V, W)$，$\dim V = n,\enspace \dim W = m$，其正奇异值按重数计为
    \[
        s_1 \geqslant s_2 \geqslant \cdots \geqslant s_r > 0.
    \]
    则存在 $V$ 的一组标准正交基 $e_1, \ldots, e_n$ 和 $W$ 的一组标准正交基 $f_1, \ldots, f_m$，使得
    \[
...
\begin{proof}
    因为 $T^*T$ 是自伴的，由实谱定理（\autoref{thm:实谱定理}），存在 $V$ 的一组标准正交基 $e_1, \ldots, e_n$，使得
    \[
    ...
    \begin{align*}
        \langle f_j, f_k \rangle
         ...
         & = \frac{1}{s_j s_k}\langle e_j, s_k^2 e_k \rangle
          ...
          = \delta_{jk}.
    \end{align*}
    因此 $f_1, \ldots, f_r$ 的确是 $W$ 的标准正交组. 对于 $k = r+1, \ldots, n$，因为 $T^*Te_k = 0$，由\autoref{thm:T^*T的性质} 第 2 条知 $e_k \in \ker T$，从而 $Te_k = 0$. 接下来将 $f_1, \ldots, f_r$ 扩充为 $W$ 的一组标准正交基 $f_1, \ldots, f_m$. 对任意 $v \in V$，由于 $e_1, \ldots, e_n$ 是 $V$ 的一组标准正交基，故
    \[
        v = \langle v, e_1 \rangle e_1 + \cdots + \langle v, e_n \rangle e_n.
    \]
    于是
    \[ Tv = \langle v, e_1 \rangle Te_1 + \cdots + \langle v, e_n \rangle Te_n = s_1 \langle v, e_1 \rangle f_1 + \cdots + s_r \langle v, e_r \rangle f_r. \]
    命题得证.
\end{proof}

由此我们也可以得出 $T$ 在这两组标准正交基下的矩阵表示. 若将 $T$ 在 $(e_1, \ldots, e_n)$ 与 $(f_1, \ldots, f_m)$ 下的矩阵记为 $\Sigma = (\sigma_{ij})_{m \times n}$，则
\[
    \sigma_{ij} =
    \begin{cases}
...
总而言之，\autoref{thm:奇异值分解定理}告诉我们：对于一般线性映射而言，虽然未必能在同一个空间里找到一组基把它化成对角矩阵，但如果允许在定义域和值域分别选取不同的标准正交基，那么它依然可以被化成极其简单的形式. 这正是奇异值分解与若当标准型并不矛盾的原因.

通过线性映射的矩阵表示过程，我们也可以得到矩阵形式的奇异值分解.

\begin{theorem}{矩阵的奇异值分解}{矩阵的奇异值分解}
    设 $A$ 为一 $m \times n$ 阶复矩阵，则存在 $m \times m$ 阶酉矩阵 $U$，$n \times n$ 阶酉矩阵 $V$ 以及 $m \times n$ 阶非负实数矩形对角矩阵 $\Sigma$，使得 $A = U \Sigma V^*$. 这称为矩阵 $A$ 的奇异值分解.

    特别地，当 $A$ 是实矩阵时，$U, V$ 可以取为正交矩阵，此时分解写成 $A = U \Sigma V^{\mathrm{T}}$.
\end{theorem}
...
\begin{proof}
    ...
    \]
    令 $U = (u_1, \ldots, u_m), V = (v_1, \ldots, v_n)$. 因为两组列向量都是标准正交基，所以 $U, V$ 均为酉矩阵（实数域上就是正交矩阵）. 设 $\Sigma$ 为 $m \times n$ 阶矩形对角矩阵，其对角线上前 $r$ 个元素依次为 $s_1, \ldots, s_r$，其余元素全为 $0$. 则
    \[
        AV = (Av_1, \ldots, Av_n) = (s_1u_1, \ldots, s_ru_r, 0, \ldots, 0) = U\Sigma.
    \]
    两边右乘 $V^*$ 即得 $A = U\Sigma V^*$，命题得证.
\end{proof}

这也表明所有矩阵的作用都可以看成三步：先在定义域中做一次保长度的变换 $V^*$，再沿着若干彼此正交的方向做伸缩 $\Sigma$，最后在值域中做一次保长度的变换 $U$.

在理解了奇异值分解的存在性及其意义后，下面的目标则是给出一个矩阵，我们能求出其奇异值分解。下面的例子基于前述讨论给出了一套求矩阵奇异值分解的通用步骤，并在具体例题中演示了如何执行这些步骤.
...
\subsubsection*{奇异值的几何解释}

此前我们已经提到，根据奇异值分解的表达式，奇异值分解把一个一般的线性映射（矩阵）分解成了旋转（或镜像）$+$ 拉伸 $+$ 旋转（或镜像）三步. 本节希望更细致地描述这一点，进而揭示奇异值的几何意义.
...
\[
    s_1 \geqslant s_2 \geqslant \cdots \geqslant s_r > 0.
\]
记 $B = \{x \in \mathbf{R}^n \mid \lVert x \rVert \leqslant 1\}$ 为 $\mathbf{R}^n$ 的单位球. 我们来研究 $T_A(B)$ 的形状，其中 $T_A(x) = Ax$.

\begin{theorem}{}{奇异值的椭球解释}
    设 $A = U\Sigma V^{\mathrm{T}} \in \mathbf{R}^{m \times n}$ 的正奇异值为 $s_1, \ldots, s_r$. 则单位球 $B$ 在 $T_A$ 下的像是位于 $\im T_A$ 中的一个（可能退化的）椭球：
    \[
        T_A(B) =
        \left\{
            U
...
\begin{proof}
    任取 $x \in B$，令 $z = V^{\mathrm{T}}x$. 由于 $V$ 是正交矩阵，所以 $\lVert z \rVert = \lVert x \rVert \leqslant 1$. 又因为 $Ax = U\Sigma V^{\mathrm{T}}x = U\Sigma z$，$U$ 是正交矩阵，代表旋转（或镜像），所以只需研究 $\Sigma$ 对单位球的作用.

    设 $z = (z_1, \ldots, z_n)^{\mathrm{T}}$，则 $\Sigma z = (s_1z_1, \ldots, s_rz_r, 0, \ldots, 0)^{\mathrm{T}}$. 若记 $y_i = s_i z_i,\enspace i = 1, \ldots, r$，则由 $z_1^2 + \cdots + z_n^2 \leqslant 1$ 可得
    \[
    ...
    这就说明 $\Sigma(B)$ 正是一个坐标轴方向上的椭球，而左乘正交矩阵 $U$ 只是把这个椭球整体旋转（或镜像）到值域中的另一个位置，不改变半轴长度. 因此 $T_A(B)$ 是一个半轴长度分别为 $s_1, \ldots, s_r$ 的椭球，且这些半轴方向恰好由 $U$ 的前 $r$ 个列向量，也就是左奇异向量给出.
\end{proof}

总结上述证明过程，右奇异向量给出了 $T_A$ 出发空间中彼此正交的一系列方向，$T_A$ 在这些方向上的伸张倍数分别就是奇异值，最后左奇异向量给出这些方向在到达空间中旋转后得到的方向. 下面的示意图展示了二维情形下单位圆 $\to$ 轴对齐椭圆 $\to$ 旋转后的椭圆的过程.
...
\begin{center}
    \begin{tikzpicture}[scale=0.9, >=latex]
        ...
        \begin{scope}[xshift=5.4cm]
            ...
            \draw[->,red] (0,0) -- ({1.7*cos(28)},{1.7*sin(28)}) node[above right] {$u_1$};
            \draw[->,red] (0,0) -- ({-0.8*sin(28)},{0.8*cos(28)}) node[above left] {$u_2$};
            \node at (0,-1.8) {再经 $U$ 旋转后};
        \end{scope}
    \end{tikzpicture}
\end{center}

在这个意义下，奇异值分解把一个一般线性映射矩阵的几何本质完全揭示了出来：它无非就是先在出发空间中调整观察方向，再沿着若干互相正交的方向做不同比例的拉伸，最后再把结果旋转到到达空间中的适当位置.
...
设 $A = U\Sigma V^{\mathrm{T}}$ 是 $A$ 的奇异值分解，其中 $U \in \mathbf{R}^{m \times m}, V \in \mathbf{R}^{n \times n}$ 为正交矩阵，$\Sigma \in \mathbf{R}^{m \times n}$ 为非负实数矩形对角矩阵. 则
\[
...
从线性代数的角度看，特征人脸算法之所以高效，正是因为奇异值分解把求大矩阵 $A^{\mathrm{T}}A$ 的主特征向量的任务，转化成了求小矩阵 $AA^{\mathrm{T}}$ 的主特征向量再回代恢复的任务. 这也是奇异值分解在数据分析中极其重要的原因之一.

\subsubsection*{算子范数}

奇异值除了刻画各个特殊方向上的伸缩倍数之外，还控制了线性映射整体上能够把向量拉多长. 这就引出算子范数的概念.
